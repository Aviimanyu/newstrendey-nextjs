...
export default function CompareClient({ lang = "en" }: CompareClientProps) {
  ...
  const handleCompare = (e: React.FormEvent) => {
    ...
    if (vehicle1 === vehicle2) {
      setError(translate("Please select two different vehicles to compare.", lang));
      return;
    }
    setError("");
    const prefix = lang === "en" ? "" : `/${lang}`;
    router.push(`${prefix}/autos/compare/${vehicle1}-vs-${vehicle2}/`);
  };

  const getLanguagePath = (id1: string, id2: string) => {
    const prefix = lang === "en" ? "" : `/${lang}`;
    return `${prefix}/autos/compare/${id1}-vs-${id2}/`;
  };
    ...
    <div className="bg-white min-h-screen py-8">
      <div className="container-custom">
        ...
        <nav className="flex items-center gap-1.5 text-xs text-text-secondary mb-8 font-bold uppercase tracking-wider max-w-4xl mx-auto">
          <Link href={lang === "en" ? "/" : `/${lang}/`} className="hover:text-brand transition-colors">
            {translate("Home", lang)}
          </Link>
          <ChevronRight className="h-3 w-3" />
          <Link href={lang === "en" ? "/autos/" : `/${lang}/autos/`} className="hover:text-brand transition-colors">
            {translate("Autos & Vehicles", lang)}
          </Link>
          <ChevronRight className="h-3 w-3" />
          <span className="text-gray-400">{translate("Compare SUVs", lang)}</span>
        </nav>
        ...
        <header className="max-w-4xl mx-auto mb-12 text-center">
          <span className="bg-brand/10 text-brand px-3 py-1 rounded-sm text-xs font-extrabold uppercase tracking-widest mb-4 inline-block">
            {translate("Compare", lang)}
          </span>
          <h1 className="font-serif text-3xl md:text-5xl font-bold leading-tight tracking-tight text-black mb-4">
            {translate("SUV & Off-Road Comparison Portal", lang)}
          </h1>
          <p className="text-lg text-text-secondary leading-relaxed font-light max-w-2xl mx-auto">
            {translate("Select any two premium off-road vehicles or utility trucks below to evaluate pricing, horsepower torque output, ground clearance, and EPA efficiency ratings.", lang)}
          </p>
        </header>
        ...
        <div className="max-w-xl mx-auto bg-surface rounded-card border border-border p-8 shadow-premium mb-16">
          <div className="flex items-center gap-2 border-b border-border pb-4 mb-6">
            ...
            <h2 className="text-sm font-extrabold uppercase tracking-widest text-black">
              {translate("Select Vehicles to Compare", lang)}
            </h2>
          </div>
          ...
          <form onSubmit={handleCompare} className="space-y-6">
            ...
            >
              ...
              {translate("Run Head-to-Head Battle", lang)}
            </button>
          </form>
        </div>

        {/* Pre-generated comparison links */}
        <section className="max-w-4xl mx-auto">
          <h2 className="font-serif text-xl font-bold text-black mb-6 text-center border-b border-border pb-3">
            {translate("Popular Head-to-Head Matchups", lang)}
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {popularComparisons.map((comp, idx) => (
              <Link
                key={idx}
                href={getLanguagePath(comp.id1, comp.id2)}
              ...
              >
                <div>
                  ...
                  <p className="text-[11px] text-text-secondary mt-1 font-light">
                    {translate("Head-to-head comparison: price, recovery clearance, and horsepower.", lang)}
                  </p>
                </div>
                <div className="mt-4 pt-3 border-t border-border flex items-center justify-between text-[10px] font-extrabold uppercase tracking-widest text-brand group-hover:translate-x-1 transition-transform">
                  <span>{translate("Compare Specs", lang)}</span>
